...
import bitcamp.myapp.service.StorageServie;
...
public class BoardController {
  ...
  private final StorageServie storageServie;
  ...
  public String add(
    ...
    if (board.getCategory() == 1) {
      for (MultipartFile file : attachedFiles) {
        ...
        String filename = storageServie.upload(bucketName, uploadDir, file);
  ...
  public String delete(int category, int no, HttpSession session) throws Exception {
    ...
    for (AttachedFile file : files) {
      storageServie.delete(this.bucketName, this.uploadDir, file.getFilePath());
    }

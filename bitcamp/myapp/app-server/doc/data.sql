...
insert into members(email,name,password,created_date)
  ...
  select b.board_no,
   ...
   from
    boards b left outer join board_files bf
...
SELECT
    ...
    b.writer,
    b.created_date,
    GROUP_CONCAT(bf.file_no) AS file_nos,
    GROUP_CONCAT(bf.file_path) AS file_paths
FROM
    boards b
INNER JOIN
    board_files bf ON b.board_no = bf.board_no
WHERE
    b.board_no = 8
GROUP BY
    b.board_no, b.title, b.writer, b.created_date;
